...
import test as validate  # for end-of-epoch mAP
from models.experimental import attempt_load
from models.yolo import Model
from utils.autoanchor import check_anchors
from utils.autobatch import check_train_batch_size
from utils.callbacks import Callbacks
from utils.dataloaders import create_kpt_dataloader
...
from utils.loss import  ComputeLoss_Kpt as ComputeLoss
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze,aux_ota_loss, kpt_label  = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
        opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze,opt.aux_ota_loss, opt.kpt_label 
    ...
    # Trainloader
    train_loader, dataset = create_kpt_dataloader(train_path,
                                              imgsz,
    ...
    # Process 0
    if RANK in {-1, 0}:
        val_loader = create_kpt_dataloader(test_path, imgsz, batch_size * 2, gs, opt,  # testloader
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                loss, loss_items = compute_loss(pred, targets.to(device)) # loss scaled by batch_siz
