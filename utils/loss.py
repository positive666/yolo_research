...
from utils.tal import dist2bbox,TaskAlignedAssigner,make_anchors,bbox2dist
import numpy as np
...
class ComputeLoss_Kap:
    # Compute losses
    def __init__(self, model, autobalance=False, num_coords=0):
        super(ComputeLoss_Kap, self).__init__()
        ...
        if self.autobalance:
            self.loss_coeffs = det = de_parallel(model).model[-1]  # Detect() module
...
class ComputeLoss_V8:
    ...
    def __call__(self, preds, batch):
        ...
        # targets
        batch_idx,batch_cls,batch_bboxes= batch.split((1, 1, 4), dim=1)
       # print(batch_idx.view(-1).shape)
       #3 print('batch_bboxe:',batch_bboxes.shape)
        
        #targets = torch.cat((batch["batch_idx"].view(-1, 1), batch["cls"].view(-1, 1), batch["bboxes"]), 1)
        targets =torch.cat((batch_idx.view(-1).view(-1,1),batch_cls.view(-1, 1),batch_bboxes),1)
