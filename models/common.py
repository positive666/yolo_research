...
class Proto(nn.Module):
    # YOLOv5 and Yolov8 mask Proto module for segmentation models
    def __init__(self, c1, c_=256, c2=32):  # ch_in, number of protos, number of masks
        super().__init__()
        self.cv1 = Conv(c1, c_, k=3)
        self.upsample = nn.ConvTranspose2d( c_,c_,2,2,0, bias=True) # add learnable layer
...
class TransformerLayer(nn.Module):    #remover LN 
...
class Detections:
    # detections class for YOLO inference results
    ...
    def __repr__(self):
        return f'YOLO_Research {self.__class__} instance\n' + self.__str__()
...
class Classify(nn.Module):
    ...
    def forward(self, x):
        if isinstance(x, list):
            x = torch.cat(x, 1)
        x=self.liear(self.drop(self.pool(self.conv(x)).flatten(1)))
