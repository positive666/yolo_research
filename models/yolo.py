...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), Decoupled=False, ch=(), inplace=True):  # detection layer
        super().__init__()
        ...
        self.m = nn.ModuleList(DecoupledHead(x,nc,anchors) for x in ch) if self.decoupled else nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in ch)  # output conv
